...
def local_sparsity_baseline_opt_wrapper(X, freq_width_energy=11, freq_width_sparsity=21, time_width_energy=11, time_width_sparsity=11, zeta = 80):
    #print(f"freq_width_sparsity = {freq_width_sparsity}\nfreq_width_energy = {freq_width_energy}\ntime_width_energy = {time_width_energy}\ntime_width_sparsity = {time_width_sparsity}\nzeta = {zeta}")
    return local_sparsity_baseline_opt(X, freq_width_energy, freq_width_sparsity, time_width_energy, time_width_sparsity, zeta)

@cython.boundscheck(False)
@cython.wraparound(False) 
@cython.nonecheck(False)
@cython.cdivision(True)
cdef local_sparsity_baseline_opt(double[:,:,::1] X_orig, Py_ssize_t freq_width_energy, Py_ssize_t freq_width_sparsity, Py_ssize_t time_width_energy, Py_ssize_t time_width_sparsity, double zeta):
    ...
    # Realiza zero-padding no tensor de espectrogramas. TODO não é mais necessário max_freq_width_lobe, poderia ser freq_width_sparsity_lobe. Mas tem que alterar no cálculo da esparsidade também.
