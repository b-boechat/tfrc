# Versão 1 otimizada do Local Sparsity interpolado, com a janela de cálculo sendo copiada em ordem crescente do valor da janela.
...
def local_sparsity_interpolation_v1_wrapper(X, freq_width_energy=15, freq_width_sparsity=39, time_width=11, zeta = 80):
    ...
    return local_sparsity_interpolation_v1(X, freq_width_energy, freq_width_sparsity, time_width, zeta)

@cython.boundscheck(False)
@cython.wraparound(False) 
@cython.nonecheck(False)
@cython.cdivision(True)
cdef local_sparsity_interpolation_v1(double[:,:,::1] X_orig, Py_ssize_t freq_width_energy, Py_ssize_t freq_width_sparsity, Py_ssize_t time_width, double zeta):
